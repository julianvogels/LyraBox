...
def all_done():
    GPIO.cleanup()
    print "GPIO Cleanup"
    #lyra_mapper.dev.free()
    #print "Libmapper cleanup"
...
def switch1_callback():
    ...
    lyra_mapper.updateSignal(2, libm_switch1_val)
...
tolerance = 10           # to keep from being jittery only change
...
while True:
        ...
        if DEBUG:
                pass
                #print "libm_sensor0_val:", libm_sensor0_val
                #print "sensor0_adjust:", sensor0_adjust
                #print "adc_last_state", adc_last_state

        if ( sensor0_adjust > tolerance ):
               libm_sensor0_val_changed = True

        if DEBUG:
                pass
                #print "libm_sensor0_val_changed", libm_sensor0_val_changed

        if ( libm_sensor0_val_changed ):
            lyra_mapper.updateSignal(6, libm_sensor0_val)

        # save the sensor reading for the next loop
        adc_last_state = libm_sensor0_val
